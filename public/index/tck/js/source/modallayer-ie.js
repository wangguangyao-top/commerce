    ...
    Object.keys(__proto__).forEach(function(attr, index) {
      ...
      if (attr === 'option') {
        ...
        Object.keys(__proto__[attr]).forEach(function(k, i) {
          if (['text', 'pageOption'].indexOf(k) >= 0)
            this.option[k] = ModalLayer.assistant.util.deepClone(__proto__[attr][k]);
          else {
            this.option[k] = __proto__[attr][k];
          }
        }, this);
      } else {
        if (!(__proto__[attr] instanceof Function)) {
          if (__proto__[attr] instanceof Object)
            this[attr] = ModalLayer.assistant.util.deepClone(__proto__[attr]);
          else
            this[attr] = __proto__[attr];
        }
      }
    }, this);
 ...
 * @type {String}
 */
ModalLayer.prototype.status = 'hide';
 ...
 * @type {Object}
 */
ModalLayer.prototype.globalVariable = {};

/**
 * 模态层使用的计时器
 * @type {Array}
 */
ModalLayer.prototype.timeoutClock = [];

/**
 * 模态层使用的间隔器
 * @type {Array}
 */
ModalLayer.prototype.intervalClock = [];
...
ModalLayer.prototype.option = {
  ...
  'callback': {
    ...
    'no': null,
    'cancel': function () {
      let self = ModalLayer.getModalLayerByDom(this);
      self.remove();
    },
    ...
    'expand': function () {
      ...
      let modalLayerNodes = self.getNodes();
      let iframeNode = modalLayerNodes.container.querySelector('iframe[name=' + self.option.pageOption.name + self.option.index + ']');
  ...
  'event': {
    ...
    active: function (downEvent) {
      ...
      self.globalVariable.mousedown = [true, nowTime];
    ...
    'drag': function (downEvent) {
      ...
      let statusText = self.status;
      let nowTime = new Date().getTime();
      let dEvent = downEvent || window.event;
      // 目标元素
      let target = self.getNodes().container;
      // 目标元素Rect
      ...
      // 父窗体边界值(左右边界值, 上下边界值)
      let boundary = [parentWindowRect.left, parentWindowRect.left + parentWindowRect.width, parentWindowRect.left, parentWindowRect.top + parentWindowRect.height]
      ...
      if (!dragMask) {
        dragMask = ModalLayer.assistant.util.objectToDom([self.nodeData.dragMask])[0];
      ...
      let mouseMoveEvent = function (moveEvent) {
        ...
        mousePoint = [mEvent.screenX, mEvent.screenY];
    ...
    'resize': function (downEvent) {
      ...
      let targetRect = target.getBoundingClientRect();
      // 目标元素最小大小
      let targetMinArea = self.globalVariable.defaultArea;
      // 鼠标按下时的坐标
      let mousePoint = [dEvent.screenX, dEvent.screenY];
      // 目标元素长宽
      ...
      if (!resizeMask) {
        resizeMask = ModalLayer.assistant.util.objectToDom([self.nodeData.resizeMask])[0];
      ...
      if (resizeBodyMask.parentNode !== document.body || resizeMask === resizeBodyMask || !resizeBodyMask) {
        resizeBodyMask = ModalLayer.assistant.util.objectToDom([self.nodeData.resizeMask])[0];
 ...
 * @param  {Object} options 初始化设置
 */
ModalLayer.prototype.initOption = function (options) {
 ...
 * @return {Object}         生成的数据
 */
ModalLayer.prototype.initBuildData = function () {
  let buildData = {};
 ...
 */
ModalLayer.prototype.initNode = function (buildData) {
  // class
  ...
  if (this.option.type === 'page') {
    ...
    pageNode.style.cssText = pageStyle;
  }
  ...
  modalLayerNodeKeys.forEach(function (key) {
    ...
    // 设置过渡样式
    modalLayerNodes[key].style.cssText = opacityStyle + transitionStyle;
 ...
 */
ModalLayer.prototype.initContent = function (container) {
 ...
 */
ModalLayer.prototype.initEvent = function (modalLayerNodes) {
  let self = this;
  // 点击遮罩层移除模态层
 ...
 * 停止所有间隔器
 */
ModalLayer.prototype.stopAllInterval = function () {
 ...
 */
ModalLayer.prototype.stopInterval = function (i) {
 ...
 * 停止所有计时器
 */
ModalLayer.prototype.stopAllTimeout = function () {
 ...
 * @param  {Number} i 间隔器索引
 */
ModalLayer.prototype.stopTimeout = function (i) {
 ...
 * 移除事件
 */
ModalLayer.prototype.removeAllEvent = function () {
  let modalLayerNodes = this.getNodes();
  if (Object.keys(modalLayerNodes).length === 0) return false;

  // 移除遮罩层点击事件
  if (this.option.mask)
    modalLayerNodes.mask.removeEventListener('click', this.option.callback.clickMask);
  ...
  let cancelButton = modalLayerNodes.container.querySelector('.modal-layer-button-cancel');
  if (okButton)
    okButton.removeEventListener('click', this.option.callback.ok);
  if (noButton)
    noButton.removeEventListener('click', this.option.callback.no);
  if (cancelButton)
...
/**
 * 通过DOM对象获取当前模态层对象
 * @param  {Node} node DOM对象(属于模态层)
 * @return {Node}      ModalLayer对象
 */
ModalLayer.__proto__.getModalLayerByDom = function (node) {
  let modalIndex = null;
  let classList = node.classList;
  let indexClass = 'modal-layer-index-';
  ...
  for (let i = 0, len = classList.length; i < len; i++) {
    ...
    if (item.indexOf(indexClass) >= 0) {
      modalIndex = item.replace(indexClass, '');
      break;
    }
  }
 ...
 * @return {Object}
 */
ModalLayer.prototype.getNodes = function () {
  let nodes = {};
 ...
 */
ModalLayer.prototype.setStatus = function (statusText) {
...
* 根据屏幕大小重绘模态层大小
*/
ModalLayer.prototype.resize = function () {
  let newModalWidth = 0;
 ...
 * @param {Number} h 容器高
 */
ModalLayer.prototype.resizeByXYWH = function (x, y, w, h) {
  let containerNode = this.getNodes().container;
 ...
 * 展示模态层
 */
ModalLayer.prototype.show = function () {
  let showStatus = 'show';
  let showCls = 'modal-layer-show';
  let hideCls = 'modal-layer-hide';
  let modalLayerNodes = this.getNodes();
 ...
 */
ModalLayer.prototype.hide = function (callback) {
  let self = this;
  let hideStatus = 'hide';
  let hideCls = 'modal-layer-hide';
  let showCls = 'modal-layer-show';
  ...
  * 最小化模态层
  */
ModalLayer.prototype.minimize = function () {
  let title = null;
  let maxTaskItem = 8;
  ...
  let itemList = taskbarNode.querySelectorAll('.' + taskItemNode.className)
  for (let i = 0, itemCount = itemList.length; i < itemCount; i++) {
    itemList[i].style.width = taskItemWidth;
  }

  let tmpClock = setInterval(function () {
  ...
  * 还原模态层(处于特殊状态)
  */
ModalLayer.prototype.revert = function () {
  let showText = 'show';
  let minimizeText = 'minimize';
 ...
 * 移除所有模态层
 */
ModalLayer.__proto__.removeAll = function () {
 ...
 */
ModalLayer.__proto__.msg = function (options) {
  let msgLayer = null;
 ...
 * @return {ModalLayer}     模态层实例
 */
ModalLayer.__proto__.alert = function (options) {
  let alertLayer = null;
 ...
 * @return {ModalLayer}     模态层实例
 */
ModalLayer.__proto__.prompt = function (options) {
 ...
 * @return {ModalLayer}     模态层实例
 */
ModalLayer.__proto__.confirm = function (options) {
 ...
 * @return {ModalLayer}     模态层实例
 */
ModalLayer.__proto__.page = function (options) {
  let pageLayer = null;
 ...
 * @type {Array}
 */
ModalLayer.__proto__.instance = [];

/**
 * 最小化模态层实例
 * @type {Array}
 */
ModalLayer.__proto__.minimizeList = [];

/**
 * 工具类集合
 * @type {Object}
 */
ModalLayer.__proto__.assistant = {
  'util': Util
};
